    ...
    if(key_value || pre_key)
    {
       switch(key_value){
		...
		case 4:
			...
			gt1x_touch_down(input_x, input_y, input_w,id);
			break;
		#elif (defined(CONFIG_BOARD_CAMELLIA)||defined(CONFIG_BOARD_BREAKER))
		case 1:
			input_x  = 85;
			input_y  = 921;
			input_w  = 77;
...
static int gt1x_smart_cover_change_config(void)
{
	s32 ret = 0;
	u8 cfg_len = 0;
	u8 sensor_id = 0;

	const u8 cfg_grp0[] = GTP_SMART_COVER_CFG_GROUP0;
	const u8 cfg_grp1[] = GTP_SMART_COVER_CFG_GROUP1;
	const u8 cfg_grp2[] = GTP_SMART_COVER_CFG_GROUP2;
	const u8 cfg_grp3[] = GTP_SMART_COVER_CFG_GROUP3;
	const u8 cfg_grp4[] = GTP_SMART_COVER_CFG_GROUP4;
	const u8 cfg_grp5[] = GTP_SMART_COVER_CFG_GROUP5;
	const u8 *cfgs[] = {
		cfg_grp0, cfg_grp1, cfg_grp2,
		cfg_grp3, cfg_grp4, cfg_grp5
	};
...
int gt1x_smart_cover_init(void)
{
	...
	*/
    //gt1x_smart_cover_update_state();
    gt1x_smart_cover_change_config();
...
#if defined (CONFIG_BOARD_PEONY)
	printk("smart cover, gt1x_resume, hall_state = %d\n", hall_state);
	if(hall_state == 1){
