...
static int fwu_get_device_config_id(void)
{
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
				...
				fwu->config_id,
				config_id_size);
	printk("%s: f34_fd.ctrl_base_addr = 0x%x\n",__func__, fwu->f34_fd.ctrl_base_addr);
...
static enum flash_area fwu_go_nogo(void)
{
	...
	printk("%s: get_config_id_addr = 0x%x\n",__func__, get_config_id_addr);
	if (retval < 0)
	...
	printk("%s: ready_fw_version = %d\n", __func__, ready_fw_version);

	for (ii = 0; ii < config_id_size; ii++) {
		printk("%s: syna_fwu->img.ui_config.data[%d] = 0x%x, fwu->config_id[%d] = 0x%x\n",
