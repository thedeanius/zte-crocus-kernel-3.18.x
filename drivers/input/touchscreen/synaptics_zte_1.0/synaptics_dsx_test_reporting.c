...
static void synaptics_rmi4_f54_status_work(struct work_struct *work)
{
	...
	printk("%s:start to get report data!\n", __func__);
	if (f54->status != STATUS_BUSY)
		return;
	...
	printk("%s: read report data successfully!\n", __func__);
...
int synaptics_rmi4_raw_data_config(int report_type)
{
	...
	if (f54->status != STATUS_BUSY) {
		...
		data = (unsigned char)f54->report_type;
		retval = f54->fn_ptr->write(rmi4_data,
				f54->data_base_addr,
				&data,
				sizeof(data));
