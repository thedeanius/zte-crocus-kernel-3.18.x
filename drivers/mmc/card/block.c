...
#define MMC_BLK_PARTSWITCH	BIT(5)
	...
	struct device_attribute wp_status;
...
static ssize_t
write_protected_status_show(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	struct mmc_blk_data *md = mmc_blk_get(dev_to_disk(dev));
	int ret;
...
static void mmc_blk_remove_req(struct mmc_blk_data *md)
{
	...
	if (md) {
		...
		device_remove_file(disk_to_dev(md->disk), &md->wp_status);
...
static int mmc_add_disk(struct mmc_blk_data *md)
{
	...
	md->wp_status.show = write_protected_status_show;
	sysfs_attr_init(&md->wp_status.attr);
	md->wp_status.attr.name = "write_protected";
	md->wp_status.attr.mode = S_IRUGO;
	ret = device_create_file(disk_to_dev(md->disk), &md->wp_status);
	if (ret)
		goto wp_status_fail;
...
void do_set_partition_ro(struct mmc_card *card, struct mmc_blk_data *md)
{
	...
	if (strstr(saved_command_line, RECOVERY_MODE_STR)) {
		...
		set_partition_ro(card,md,"sbl1",12); /*sbl1+DDR+rpm+tz+devcfg+fsg+fingerid+dsp+splash+echarge+devinfo+limits */
		set_partition_ro(card,md,"aboot",8); /*aboot+modem+boot+recovery+lksecapp+cmnlib+cmnlib64+keymaster */
