...
static struct workqueue_struct *gt1x_smart_cover_wq;
static struct work_struct	gt1x_smart_cover_work;
#endif
...
#if defined (CONFIG_BOARD_PEONY)
void gt1x_smart_cover(bool enable)
{
	hall_state = enable;
	printk("%s:hall_state = %d\n", __func__, hall_state);

	if (!gt1x_smart_cover_wq)
		printk("%s:gt1x_smart_cover_wq is NULL and failed to enable smart cover!\n", __func__);
	else
		queue_work(gt1x_smart_cover_wq, &gt1x_smart_cover_work);

	return;
}

void gt1x_smart_cover_enable(struct work_struct *work)
{
	printk("%s:hall_state=%d\n", __func__, hall_state);
	...
	if (hall_state){
		...
		gt1x_init_panel(); //zte_tzb add to solve TP can not use, panel on first then open smart cover, 20160716
	}
...
static int gt1x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* add by zte_tzb for smart_cover, 20160706 */
#if defined (CONFIG_BOARD_PEONY)
       gt1x_smart_cover_wq=create_singlethread_workqueue("gt1x_smart_cover_wq");
	if (!gt1x_smart_cover_wq){
		printk("Could not create work queue gt1x_smart_cover_wq: no memory");
	}
	INIT_WORK(&gt1x_smart_cover_work, gt1x_smart_cover_enable);
#endif
	/* add by zte_tzb for smart_cover, 20160706 */
