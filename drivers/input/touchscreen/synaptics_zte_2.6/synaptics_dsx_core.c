		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (button = 0; button < f1a->valid_button_count; button++) {
		...
		printk("%s: Button %d (code %d) ->%d\n",
				...
				status);
#if defined(CONFIG_BOARD_PEONY) || defined(CONFIG_BOARD_PROLINE) || defined(CONFIG_BOARD_WARP8)
		switch (button) {
		case 0:
			x = 128;
			break;
	...
	int *p_fw_ver )
{
	...
	case 'H':
		...
		break;
	case 'I':
		snprintf(p_sensor, PAGE_SIZE, "DIJING(0x%x)", ts->config_id.sensor);
		touch_module = DIJING;
		break;
	case 'R':
		snprintf(p_sensor, PAGE_SIZE, "HELITECH(0x%x)", ts->config_id.sensor);
	...
	*p_fw_ver = ts->config_id.fw_ver;
	...
	dev_info(ts->pdev->dev.parent,
		"%s: chip = %s, sensor = %s, fw = 0x%x\n",
		__func__, p_chip_type, p_sensor, *p_fw_ver);
...
static int get_max_std_rawdata(short *max_rawdata)
{
	...
	short rawdata11[] = SYN_EACHOPTO_RAW_DATA_MAX;
	short rawdata12[] = SYN_MUTTO_RAW_DATA_MAX;
	short rawdata13[] = SYN_JUNDA_RAW_DATA_MAX;
	short rawdata14[] = SYN_BOE_RAW_DATA_MAX;
	short rawdata15[] = SYN_TIANMA_RAW_DATA_MAX;
	short rawdata16[] = SYN_SAMSUNG_RAW_DATA_MAX;
	short rawdata17[] = SYN_DIJING_RAW_DATA_MAX;
	short rawdata18[] = SYN_LCE_RAW_DATA_MAX;
	short rawdata19[] = SYN_GUOXIAN_RAW_DATA_MAX;
	short rawdata20[] = SYN_HELITECH_RAW_DATA_MAX;
	short *rawdata[] = {
		rawdata0, rawdata1, rawdata2, rawdata3, rawdata4, rawdata5,
		rawdata6, rawdata7, rawdata8, rawdata9, rawdata10, rawdata11,
		rawdata12, rawdata13, rawdata14, rawdata15, rawdata16, rawdata17,
		rawdata18, rawdata19, rawdata20
	};
	...
	short rawdata_lens[] = {
		...
		RAWDATA_LEN(rawdata16),
		RAWDATA_LEN(rawdata17),
		RAWDATA_LEN(rawdata18),
		RAWDATA_LEN(rawdata19),
		RAWDATA_LEN(rawdata20)
	};

	if (rawdata_lens[touch_module] != num_of_rx * num_of_tx * sizeof(short)) {
		pr_err("%s: array length is not valid, rawdata_lens=%d\n",
			__func__, rawdata_lens[touch_module]);
		return -EINVAL;
	}

	memset(max_rawdata, 0, rawdata_lens[touch_module]);
	mutex_lock(&exp_data.mutex);
	memcpy(max_rawdata, rawdata[touch_module], rawdata_lens[touch_module]);
	mutex_unlock(&exp_data.mutex);

	pr_notice("%s: max_rawdata = %d, len = %d\n",
		__func__, (int)sizeof(max_rawdata),
		(int)rawdata_lens[touch_module]);
...
static int get_min_std_rawdata(short *min_rawdata)
{
	...
	short rawdata11[] = SYN_EACHOPTO_RAW_DATA_MIN;
	short rawdata12[] = SYN_MUTTO_RAW_DATA_MIN;
	short rawdata13[] = SYN_JUNDA_RAW_DATA_MIN;
	short rawdata14[] = SYN_BOE_RAW_DATA_MIN;
	short rawdata15[] = SYN_TIANMA_RAW_DATA_MIN;
	short rawdata16[] = SYN_SAMSUNG_RAW_DATA_MIN;
	short rawdata17[] = SYN_DIJING_RAW_DATA_MIN;
	short rawdata18[] = SYN_LCE_RAW_DATA_MIN;
	short rawdata19[] = SYN_GUOXIAN_RAW_DATA_MAX;
	short rawdata20[] = SYN_HELITECH_RAW_DATA_MAX;
	short *rawdata[] = {
		rawdata0, rawdata1, rawdata2, rawdata3, rawdata4, rawdata5,
	...
	memset(min_rawdata, 0, rawdata_lens[touch_module]);
	mutex_lock(&exp_data.mutex);
	memcpy(min_rawdata, rawdata[touch_module], rawdata_lens[touch_module]);
	mutex_unlock(&exp_data.mutex);

	pr_notice("%s: min_rawdata = %d, len = %d\n",
...
ts_rawdata_test_val(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	...
	char coordinate_test_fail[num_of_rx * num_of_tx * 2];
	...
	if (!f54 || !syn_ts) {
		pr_err("%s: f54 or syn_ts is null\n", __func__);
		type_fail = -1;
		goto exit;
	}
	...
	if (retval < 0) {
		pr_err("%s: fail to get max std rawdata\n", __func__);
		type_fail = -2;
		goto exit;
	}
	...
	if (retval < 0) {
		pr_err("%s: fail to get min std rawdata\n", __func__);
		type_fail = -2;
		goto exit;
	}
	...
	if (retval < 0) {
		pr_err("%s: fail to get raw data\n", __func__);
		type_fail = -1;
		goto exit;
	}

	while (!rawdata_flag && (count > 0)) {
		usleep_range(1000, 1500);
		count--;
	}

	if (!f54->report_data) {
		pr_err("%s: f54->report_data is null\n", __func__);
		type_fail = -1;
		goto exit;
	}
	...
	for (ii = 0; ii < num_of_rx * num_of_tx; ii++) {
		if (*(max_rawdata + ii) < *(report_data_16 + ii)) {
			pr_notice("%s: test fail! index = %d, max_rawdata = %d, *report_data_16 = %d\n",
				__func__, ii, *(max_rawdata + ii), *(report_data_16 + ii));
			if (((ii + 1) % num_of_rx) == 0)
				type_fail |= MAX_0D_FAIL;
			else {
				type_fail |= MAX_2D_FAIL;
				coordinate_test_fail[num_fail * 2] = ii / num_of_rx;
				coordinate_test_fail[num_fail * 2 + 1] = ii % num_of_rx;
				num_fail++;
			}
		}
		if (*(min_rawdata + ii) > *(report_data_16 + ii)) {
			pr_notice("%s: test fail! index = %d, min_rawdata = %d, *report_data_16 = %d\n",
				__func__, ii, *(min_rawdata + ii), *(report_data_16 + ii));
			if (((ii + 1) % num_of_rx) == 0)
				type_fail |= MIN_0D_FAIL;
			else {
				type_fail |= MIN_2D_FAIL;
				coordinate_test_fail[num_fail * 2] = ii / num_of_rx;
				coordinate_test_fail[num_fail * 2 + 1] = ii % num_of_rx;
				num_fail++;
			}
...
exit:
	/*retval = synaptics_rmi4_reset_device(syn_ts, false);
	if (retval < 0) {
		printk("%s: fail to reset device\n", __func__);
	}*/

	if (num_fail) {
		len += snprintf(page + len, PAGE_SIZE - len, "%d,%d,%d,%d,%d",
			0, type_fail, num_of_tx, num_of_rx, num_fail);
		for (ii = 0; ii < num_fail; ii++)
			len += snprintf(page + len, PAGE_SIZE - len, ",%d,%d",
				num_of_tx - 1 - coordinate_test_fail[ii * 2],
				coordinate_test_fail[ii * 2 + 1]);
		len += snprintf(page + len, PAGE_SIZE - len, "%c\n", '\0');
	} else if (type_fail)
		len += snprintf(page + len, PAGE_SIZE - len, "%d%c\n", type_fail, '\0');
	else
		len += snprintf(page + len, PAGE_SIZE - len, "%d%c\n", 1, '\0');

	if (num_fail || type_fail)
		pr_notice("%s: rawdata test fail\n", __func__);
	else
		pr_notice("%s: rawdata test pass\n", __func__);
...
ts_rawdata_read_val(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	...
	for (ii = 0; ii < num_of_rx; ii++)
		len += snprintf(page + len, PAGE_SIZE - len, "   %2d", ii);
	len += snprintf(page + len, PAGE_SIZE - len, "\n");

	for (ii = 0; ii < num_of_tx; ii++) {
		len += snprintf(page + len, PAGE_SIZE - len, "%2d ", ii);
		for (jj = 0; jj < num_of_rx; jj++)
			len += snprintf(page + len, PAGE_SIZE - len, "%5d", *(report_data_16 + ii * num_of_rx + jj));
		len += snprintf(page + len, PAGE_SIZE - len, "\n");
	}
...
static int rx_to_rx_test(int report_type)
{
	...
	retval = synaptics_rmi4_raw_data_config(report_type);
	if (retval == 1) {
		pr_notice("%s: invalid report type\n", __func__);
		test_result = 1;
		goto exit;
	} else if (retval < 0) {
		pr_err("%s: failed to get raw data\n", __func__);
	...
	for (ii = 0; ii < num_of_tx; ii++) {
		pr_notice("%2d  ", ii);
		for (jj = 0; jj < num_of_rx; jj++)
			pr_notice("%5d", *(report_data_16 + ii * num_of_rx + jj));
		pr_notice("\n");
	}

	/*compare*/
	if (report_type == F54_RX_TO_RX_SHORTS_1) {
...
exit:
	...
	if (test_result == 0)
		pr_notice("%s: test pass!\n", __func__);
	else if (test_result == 1)
		pr_notice("%s: not supported report type!\n", __func__);
	else
		pr_notice("%s: test fail!\n", __func__);
...
ts_short_circuit_test_val(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	...
	test_result = rx_to_rx_test(F54_RX_TO_RX_SHORTS_1);
	if (test_result)
		goto exit;

	if (num_of_rx > num_of_tx) {
		test_result = rx_to_rx_test(F54_RX_TO_RX_SHORTS_2);
		if (test_result)
			goto exit;
	}

	test_result = tx_to_tx_test(F54_TX_TO_TX_SHORTS);

exit:
	if (test_result < 0)
		test_result = SHORT_CIRCUIT_FAIL;
	len += snprintf(page + len, PAGE_SIZE, "%d\n", test_result < 0 ? 0X400 : 1);
...
#ifdef TP_LOG
	 refresh = proc_create("touch_log", 0644, dir, &proc_ops_read_log);
	 if (!refresh)
		pr_notice("proc_create touch_log failed!\n");
	 else
	    pr_notice("proc_create touch_log!\n");
#endif

#if RAWDATA_TEST
	refresh = proc_create("ts_rawdata_read", 0664, dir, &rawdata_read_proc_ops);
	if (!refresh)
		pr_err("proc_create ts_rawdata_read failed!\n");
	refresh = proc_create("ts_rawdata_test", 0664, dir, &rawdata_test_proc_ops);
	if (!refresh)
		pr_err("proc_create ts_rawdata_test failed!\n");
	refresh = proc_create("ts_short_circuit_test", 0664, dir, &short_circuit_test_proc_ops);
	if (!refresh)
		pr_err("proc_create ts_short_circuit_test failed!\n");
#endif
...
{
	int retval;
	unsigned short smart_cover_reg_addr = 0x1E;
	unsigned char smart_cover_reg_enable = 0x03;
	unsigned char smart_cover_reg_disable = 0x00;
	unsigned short config_reg_addr = 0x141;
	unsigned char config_reg_value = 0x04;
	unsigned char smart_cover_reg_readback = 0x00;

	if (enable) {
		retval = synaptics_rmi4_reg_write(rmi4_data,
				smart_cover_reg_addr,
				&smart_cover_reg_enable,
				sizeof(smart_cover_reg_disable));
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to write smart_cover_reg_addr\n",
				__func__);
			return;
		}
		rmi4_data->enable_smart_cover = true;
		dev_info(rmi4_data->pdev->dev.parent,
			"%s: enable smart cover\n",
			__func__);
	} else {
		retval = synaptics_rmi4_reg_write(rmi4_data,
				smart_cover_reg_addr,
				&smart_cover_reg_disable,
				sizeof(smart_cover_reg_disable));
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to write smart_cover_reg_addr\n",
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,
				config_reg_addr,
				&config_reg_value,
				sizeof(config_reg_value));
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
			"%s: Failed to write config_reg_addr\n",
			__func__);
		return;
	}
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			smart_cover_reg_addr,
			&smart_cover_reg_readback,
			sizeof(smart_cover_reg_readback));
	dev_info(rmi4_data->pdev->dev.parent,
			"%s: smart_cover_reg_readback = 0x%x\n",
			__func__, smart_cover_reg_readback);
...
/*ergate*/
	...
	if (rmi4_data->enable_wakeup_gesture) {
		...
		/* tzb set rst gpio low when suspend for P817A22, 20170104 */
		#if defined(CONFIG_BOARD_PEONY)
		gpio_direction_output(bdata->reset_gpio, 0);
		#else
		#endif
		/* tzb set rst gpio low when suspend for P817A22, 20170104 */
		msleep(20);
		/*if(syna_wake_gesture_flag == false){
			...
			#if defined(CONFIG_BOARD_PEONY) || defined(CONFIG_BOARD_STOLLEN)
