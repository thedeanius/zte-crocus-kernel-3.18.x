...
#include <linux/power/bq27426_gmfs.h>
...
static void fg_print_buf(const char *msg, u8 *buf, u8 len)
{
	...
	for (i = 0; i < len; i++) {
		...
		num = snprintf(&strbuf[idx], 2, "%02X ", buf[i]);
		idx += num;
	}
	pr_err("%s\n", strbuf);
}
					...
					union power_supply_propval *val)
{
	...
	if (time_is_before_jiffies(bq->last_update + 2 * HZ)) {
		cancel_delayed_work_sync(&bq->monitor_work);
		fg_monitor_workfunc(&bq->monitor_work.work);
	}
...
static bool fg_check_update_necessary(struct bq_fg_chip *bq)
{
	int ret;
...
static void fg_update_bqfs(struct bq_fg_chip *bq)
{
	...
	pr_info("Fuel Gauge parameter update,Start...\n");
...
static void fg_refresh_status(struct bq_fg_chip *bq)
{
	...
	last_batt_inserted	= bq->batt_present;
	last_batt_fc		= bq->batt_fc;
	last_batt_ot		= bq->batt_ot;
	last_batt_ut		= bq->batt_ut;
	last_allow_chg		= bq->allow_chg;
	...
	if (!last_batt_inserted && bq->batt_present) {/* battery inserted */
		pr_info("Battery inserted\n");
	} else if (last_batt_inserted && !bq->batt_present) {/* battery removed */
	...
	if ((last_batt_inserted != bq->batt_present)
		|| (last_batt_fc != bq->batt_fc)
		|| (last_batt_ot != bq->batt_ot)
		|| (last_batt_ut != bq->batt_ut)
		|| (last_allow_chg != bq->allow_chg))
	...
	bq->last_update = jiffies;

}

static unsigned int poll_interval = 60;
/* module_param(poll_interval, uint, 0644);
MODULE_PARAM_DESC(poll_interval, "battery poll interval in seconds - 0 to disable polling."); */
...
static void fg_monitor_workfunc(struct work_struct *work)
{
	...
	fg_refresh_status(bq);

	if (poll_interval > 0) {
		set_timer_slack(&bq->monitor_work.timer, poll_interval * HZ / 4);
