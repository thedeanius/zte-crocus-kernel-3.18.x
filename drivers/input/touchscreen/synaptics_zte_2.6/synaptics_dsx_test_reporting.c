...
unsigned char num_of_rx = 0;
unsigned char num_of_tx = 0;
bool valid_report_type = true;
#endif
...
static bool test_report_type_valid(enum f54_report_types report_type)
{
	...
	case F54_TRX_SHORTS_NEW:
	case F54_TRX_VDD_SHORTS:
		return true;
		break;
	default:
	...
	if (f54->status == STATUS_BUSY) {
		...
		if (retval < 0) {
			...
			dev_err(rmi4_data->pdev->dev.parent,
					...
					__func__);
#ifdef RAWDATA_TEST
			valid_report_type = false;
			pr_notice("%s: valid_report_type=%d\n",
				__func__, valid_report_type);
#endif
		} else {
...
static void test_report_work(struct work_struct *work)
{
	...
	pr_notice("%s: report data addr = 0x%x, size = %d\n", __func__,
		f54->data_base_addr + REPORT_DATA_OFFSET, f54->report_size);
	retval = synaptics_rmi4_reg_read(rmi4_data,
...
static void test_f55_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	pr_notice("%s: num_of_rx=%d, num_of_tx=%d\n",
		__func__, num_of_rx, num_of_tx);
#endif
...
#if RAWDATA_TEST
static ssize_t report_type_write(int report_type)
{
	int retval;
	unsigned char data;
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	...
	if (!test_report_type_valid((enum f54_report_types)report_type)) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Report type not supported by driver\n",
				__func__);
		retval = -EINVAL;
	...
	f54->report_type = (enum f54_report_types)report_type;
...
static ssize_t do_preparation(void)
{
	int retval;
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
...
static ssize_t get_report(void)
{
	int retval;
	unsigned char command;
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
...
static ssize_t resume_touch(void)
{
	int retval;
	unsigned char device_ctrl;
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
...
int synaptics_rmi4_raw_data_config(int report_type)
{
	int retval;
	unsigned char timeout = GET_REPORT_TIMEOUT_S * 10;
	unsigned char timeout_count;
	...
	retval = report_type_write(report_type);
	if (retval < 0)
		goto exit;
	...
	if (!valid_report_type) {
		pr_notice("%s: invalid report type\n", __func__);
		retval = 1;
		goto exit;
	} else if ((f54->status != STATUS_IDLE) || (f54->report_size == 0)) {
		dev_err(rmi4_data->pdev->dev.parent,
				"%s: Failed to read report\n",
				__func__);
		retval = -EINVAL;
