	...
	int *p_fw_ver )
{
	...
	if (was_in_bl_mode && ts->flash_prog_mode) {
		...
		case 50:
			...
			break;
		case 51:
			ts->config_id.sensor = 81;
			snprintf(p_sensor, PAGE_SIZE, "GUOXIAN(0x%x)", ts->config_id.sensor);
			touch_moudle = GUOXIAN;
			break;
		default:
...
static int get_max_std_rawdata(short *max_rawdata)
{
	...
	if (rawdata_lens[touch_moudle] != num_of_rx * num_of_tx * sizeof(short)) {
		printk("%s: array length is not valid\n", __func__);
		return -EINVAL;
	}
...
ts_rawdata_test_val(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	...
	if (retval < 0) {
		printk("%s: faile to get max std rawdata\n", __func__);
		type_fail = -2;
		goto exit;
	}
	...
	if (retval < 0) {
		printk("%s: faile to get min std rawdata\n", __func__);
		type_fail = -2;
		goto exit;
	}
	...
	for (ii = 0; ii < num_of_rx * num_of_tx; ii++) {
		if (*(max_rawdata + ii) < *(report_data_16 + ii)) {
			printk("%s: test fail! index = %d, max_rawdata = %d, *report_data_16 = %d\n",
		...
		if (*(min_rawdata + ii) > *(report_data_16 + ii)) {
			printk("%s: test fail! index = %d, min_rawdata = %d, *report_data_16 = %d\n",
...
ts_rawdata_read_val(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	...
	retval = synaptics_rmi4_raw_data_config(report_type);
	if (retval < 0) {
		printk("%s: faile to get raw data\n", __func__);
		goto exit;
	}
	...
	for (ii = 0; ii < num_of_rx; ii++)
		len += sprintf(page + len, "     %2d", ii);
	len += sprintf(page + len, "\n");

	for (ii = 0; ii < num_of_tx; ii++) {
		len += sprintf(page + len, "%2d ", ii);
		for (jj = 0; jj < num_of_rx; jj++) {
			len += sprintf(page + len, "  %5d", *(report_data_16 + ii * num_of_rx + jj));
		}
		len += sprintf(page + len, "\n");
	}
...
static int rx_to_rx_test(int report_type)
{
	...
	for (ii = 0; ii < num_of_tx; ii++) {
		printk("%2d  ", ii);
		for (jj = 0; jj < num_of_rx; jj++) {
			printk("  %5d", *(report_data_16 + ii * num_of_rx + jj));
		}
		printk("\n");
	}
