...
static int ois_debugfs_datatype_s(void *data, u64 val)
{
	...
	CDBG("%s:%d: msm_ois_reg_data_type = %d\n", __func__,
		__LINE__, ptr->msm_ois_reg_data_type);
...
static int sensor_debugfs_datatype_g(void *data, u64 *val)
{
	...
	*val = ptr->msm_sensor_reg_data_type;
	...
	CDBG("%s:%d: msm_sensor_reg_data_type = %d\n", __func__,
		 __LINE__, ptr->msm_sensor_reg_data_type);
...
static int sensor_debugfs_setvalue(void *data, u64 val)
{
	...
	if (rc < 0) {
		...
		__LINE__, ptr->s_ctrl->sensordata->sensor_name, val);
		return rc;
	}
...
static int ois_debugfs_setvalue(void *data, u64 val)
{
	...
	msm_ois_debug_info_t *ptr = (msm_ois_debug_info_t *) data;
	struct msm_camera_i2c_client *ois_i2c_client =
		&(ptr->s_ctrl->i2c_client);
	int32_t rc = 0;

	CDBG("%s:%d: address = 0x%llx  value = 0x%llx\n", __func__,
	__LINE__, ptr->address, val);
...
int msm_sensor_enable_debugfs(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (!debugfs_base) {
		...
		if (!debugfs_base) {
			ret = -EINVAL;
			goto msm_sensor_enable_debugfs_exit;
		}
	}
	...
	sensor_dir = debugfs_create_dir(s_ctrl->sensordata->sensor_name,
		debugfs_base);
	if (!sensor_dir) {
		ret = -EINVAL;
		goto msm_sensor_enable_debugfs_exit;
...
msm_sensor_enable_debugfs_exit:
	kfree(debug_ptr);
...
static msm_sensor_sysdev_info_t
	msm_sensor_sysdev_info[MSM_SENSOR_SYSDEV_NUM_MAX];
...
static ssize_t show_msm_sensor_id(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	...
	if (dev->kobj.name) {
		...
		rc = (int32_t)kstrtol(ptr, 10, &index);
		if (rc < 0) {
			CDBG("kstrtol called failed rc=%d\n", rc);
			return rc;
		}
...
static ssize_t show_msm_sensor_name(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	uint16_t len = 0;
	long index;
	...
	len += snprintf(buf+len, sizeof(msm_sensor_sysdev_info[index].name), "%s\n",
...
static ssize_t show_msm_sensor_position(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	uint16_t len = 0;
	long index;
	...
	len += snprintf(buf+len,  sizeof(msm_sensor_sysdev_info[index].position), "%d\n",
...
#endif
static ssize_t show_msm_sensor_checksum(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	uint16_t len = 0;
	long index;
	...
	len += snprintf(buf+len,  sizeof(msm_sensor_sysdev_info[index].checksum), "%x\n",
...
static ssize_t show_msm_sensor_flag(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	uint16_t len = 0;
	long index = 0;
	...
	len += snprintf(buf+len, sizeof(msm_sensor_sysdev_info[index].valid_flag), "%x\n",
...
void msm_sensor_register_sysdev(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	msm_sensor_sysdev_info[index].id =
...
register_sysdev_error:
	...
	while (--i  >= 0)
		device_remove_file(&device_camera_sensor[index],
			msm_sensor_dev_attrs[i]);
