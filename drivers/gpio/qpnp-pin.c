...
struct	spmi_device	*g_spmi_dev = NULL;
#endif
...
#if defined(CONFIG_BOARD_VIOLET)
void zte_spmi_write(void)
{
	u8 value = 0x80;    /* enable pm gpio led  */

	if (g_spmi_dev) {
		spmi_ext_register_writel(g_spmi_dev->ctrl, 0, 0xC646, &value, 1);
		spmi_ext_register_writel(g_spmi_dev->ctrl, 0, 0xC746, &value, 1);
		pr_info("zte_spmi_write\n");
	}
}
#endif
